document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        const resistivityCopper = 0.0168 * 1.09; 
        const resistivityAluminum = 0.0265 * 1.09;
        const resistivity = metalType === 'copper' ? resistivityCopper : resistivityAluminum;
        
        // Видаляємо множник lengthMultiplier = 2, щоб розрахувати опір одного проводу
        // let lengthMultiplier = 2; - це стара версія
        let pairFactor = 1;
        ...
        // Розраховуємо опір без множника 2
        const resistance = (resistivity * cableLength) / sectionValue * pairFactor;
        
        // Розраховуємо загальний опір кабелю (для двох провідників)
        const totalResistance = resistance * 2;
        
        // Розрахунок падіння напруги та втрат потужності
        let voltageLoss, powerLoss;
        
        if (voltageType === 'ac') {
            // Типове значення питомого індуктивного опору для побутових кабелів (Ом/м)
            const inductiveResistance = 0.0001; 
            
            // Розрахунок синуса для кута потужності
            const sinPhi = Math.sqrt(1 - cosPhi * cosPhi);
            
            // Формула для змінного струму з врахуванням індуктивної складової
            // ΔU(в) = 2 · I(R · cosφ · L + X · sinφ · L) - для однофазного змінного струму
            voltageLoss = 2 * current * (resistance * cosPhi + inductiveResistance * cableLength * sinPhi);
            
            // Розрахунок втрат потужності з врахуванням індуктивної складової
            powerLoss = current * current * totalResistance; // Втрати лише на активному опорі
        } else {
            // Для постійного струму - без індуктивної складової
            voltageLoss = current * totalResistance;
        ...
        let resultContentHtml = `
            <div class="result-header">
                <div class="result-title"><i class="bi bi-clipboard-data"></i> Аналіз втрат у кабелі</div>
            </div>

            <table class="result-table">
                <caption style="color:#2563eb;"><i class="bi bi-ethernet"></i> Параметри кабелю</caption>
            ...
            <table class="result-table">
                <caption style="color:#2563eb;"><i class="bi bi-lightning-charge-fill"></i> Електричні параметри</caption>
        ...
        resultContentHtml += `
            ...
            <table class="result-table">
                <caption style="color:#2563eb;"><i class="bi bi-graph-down-arrow"></i> Втрати та ефективність</caption>
            ...
            <table class="result-table">
                <caption style="color:#2563eb;"><i class="bi bi-shield-check"></i> Оцінка та рекомендації</caption>
